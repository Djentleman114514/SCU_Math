...
\textbf{【答案】} $\boldsymbol a\cdot\boldsymbol b=\pm\dfrac{\sqrt3}{2}$。
\end{answerbox}
...
\textbf{【速解】} 方向向量取两平面法向量叉乘，并按与 $z$ 轴夹角为锐角单位化：
\[
\boldsymbol l_0=\frac13(1,2,2).
\]
又
\[
\nabla u(1,1,0)=\left(\frac12,\frac12,\frac12\right),
\]
故方向导数为
\[
D_{\boldsymbol l}u(1,1,0)=\nabla u(1,1,0)\cdot\boldsymbol l_0
=\frac56.
\]
...
交线：
\[
\sqrt{2-r^2}=r^2\Rightarrow r=1.
\]
体积
\[
V=\int_0^{2\pi}\int_0^1\left(\sqrt{2-r^2}-r^2\right)r\dd r\dd\theta
=\frac{(8\sqrt2-7)\pi}{6}.
\]
\textbf{【答案】}：$V=\dfrac{(8\sqrt2-7)\pi}{6}$。
...
\textbf{【思路】} 将上半圆参数化为 $x=\cos\theta,\ y=\sin\theta$，再计算第一类曲线积分。

\textbf{【速解】} 令 $x=\cos\theta,\ y=\sin\theta$，$0\le\theta\le\pi$，
\[
\dd s=\dd\theta.
\]
则
\[
\int_L(x+y)x\dd s
=\int_0^\pi(\cos\theta+\sin\theta)\cos\theta\,\dd\theta
=\int_0^\pi\cos^2\theta\,\dd\theta+\int_0^\pi\sin\theta\cos\theta\,\dd\theta
=\frac\pi2.
\]
...
取 $y=0$，$0\le x\le2$：
\[
I=\int_0^2\sqrt{1+x}\dd x
=\left.\frac23(1+x)^{3/2}\right|_0^2
=2\sqrt3-\frac23.
\]

\textbf{【答案】} $2\sqrt3-\dfrac23$。
\end{answerbox}
...
=\frac{\pi+1+\pi}{2}=\pi+\frac12.
\]
\textbf{【答案】} $S(\pi)=\pi+\dfrac12$。
